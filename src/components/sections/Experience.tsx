    ...
    >
      ...
      <div className="max-w-3xl">
        {experience.map((exp, idx) => (
          ...
          >
            {/* Timeline dot */}
            <div className="absolute -left-5 top-2 w-6 h-6 rounded-full bg-accent border-4 border-canvas shadow-md shadow-accent/30" />

            <div className="md:grid md:grid-cols-3 md:gap-8">
              <div>
                <h3 className="font-serif text-2xl text-[rgb(59_18_40)] dark:text-primary font-light mb-1">
                  {exp.role}
                </h3>
                <p className="font-sans text-sm text-accent mb-1">{exp.company}</p>
                <p className="font-sans text-xs text-[rgb(154_125_140)] dark:text-muted">{exp.period}</p>
                <p className="font-sans text-xs text-[rgb(154_125_140)] dark:text-muted mt-1">{exp.location}</p>
              </div>
